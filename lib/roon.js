...
module.exports = {
  ...
  trans: () => {
    if (core) {
      return core.services.RoonApiTransport;
    } else {
      return null;
    }
  },
  ...
  control: function() {
    const trans = this.trans(), args = Array.from(arguments);
    go('control', () => {
      trans.control.apply(core.trans, [zone].concat(args));
    });
  },
  seek: function() {
    const trans = this.trans(), args = Array.from(arguments);
    go(`seek`, () => {
      trans.seek.apply(core.trans, [zone].concat(args));
    });
  },
  standby: (cb) => {
